...
export function AppShell({ children }: { children: ReactNode }) {
  const { role, setRole, alerts } = useAppState();
      ...
      <header className="sticky top-0 z-50 border-b bg-surface/95 backdrop-blur">
        <div className="flex h-14 items-center px-4 lg:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="hidden items-center gap-2 border-r pr-3 text-xs text-muted-foreground md:flex"><span className="size-2 rounded-full bg-ok" />Live · {CITY.updated}</span>
