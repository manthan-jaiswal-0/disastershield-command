...
const INC_ID = 'mock_mu5ej7g2_botg3cx';
const RES_ID = 'mock_mu5ej7gg_0ngv5sc';
...
async function run() {
  ...
  const foundInc = homeContent.includes('AUDIT-421 Flood');
  const foundRes = homeContent.includes('AUDIT-421 Team');
    ...
    try {
      ...
      if (openBtn) {
        ...
        for (const b of resBtns) {
          const txt = await page.evaluate(el => el.textContent?.trim(), b);
          if (txt?.includes('AUDIT-421 Team')) { resBtn = b; break; }
        }
        if (resBtn) {
          await resBtn.click();
          await new Promise(r => setTimeout(r, 3000));
          ...
          const afterContent = await page.content();
          const teamVisible = afterContent.includes('AUDIT-421 Team');
          ...
          log('E_ASSIGNMENT_UI', 'FAIL', '"AUDIT-421 Team" not found in panel');
        }
      } else {
  ...
  const incR2 = await apiCall('/incidents', {
    method: 'POST',
    body: JSON.stringify({ title: 'AUDIT-421 Reject', type: 'FLOOD', description: 'Rejection test', latitude: 23.1, longitude: 80.3, severity: 'LOW' })
  });
  ...
  const incR3 = await apiCall('/incidents', {
    method: 'POST',
    body: JSON.stringify({ title: 'AUDIT-421 Gov', type: 'FLOOD', description: 'Gov gate test', latitude: 25.5, longitude: 85.5, severity: 'LOW' })
  });
  ...
  log('J_NO_LOCAL_MUTATION', 'FAIL',
    'CONFIRMED DEFECT: modifyRecommendation() (app-state.tsx ~L242) calls setIncidents() locally with no backend API call. The "Modify" recommendation path mutates React state without persisting to the backend. approve/reject/assign/updateResource all correctly await apiFetch first.');
